...
class UserServiceTest {
    ...
    @Mock
    private DefaultUserNameGenerator defaultUserNameGenerator;
    ...
    void findOrRegisterKakaoUserCreatesNewUserWhenActiveUserDoesNotExist() {
        ...
        when(defaultUserNameGenerator.generate()).thenReturn("용감한등산러1234");
        ...
        assertThat(result.getName()).isEqualTo("용감한등산러1234");
    }
    ...
    void findOrRegisterKakaoUserCreatesNewUserWhenMatchedUserIsDeleted() {
        ...
        when(defaultUserNameGenerator.generate()).thenReturn("씩씩한산악인5391");
        when(userRepository.save(any(User.class))).thenAnswer(invocation -> invocation.getArgument(0));
        ...
        assertThat(result.isDeleted()).isFalse();
        assertThat(result.getName()).isEqualTo("씩씩한산악인5391");
    }

    @Test
    void findOrRegisterAppleUserCreatesNewUserWithGeneratedName() {
        when(userRepository.findByOauthIdAndOauthProvider("apple-id", OAuthProvider.APPLE))
                .thenReturn(Optional.empty());
        when(defaultUserNameGenerator.generate()).thenReturn("빠른하이커1204");
        when(userRepository.save(any(User.class))).thenAnswer(invocation -> invocation.getArgument(0));
        ...
        assertThat(result.getName()).isEqualTo("빠른하이커1204");
    }
