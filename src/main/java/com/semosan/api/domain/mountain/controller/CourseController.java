...
public class CourseController implements CourseControllerDocs {
    ...
    @Override
    public ResponseEntity<ApiResponse<Void>> likeCourse(
            @AuthenticationPrincipal Long userId,
            @PathVariable Long courseId
    ) {
        courseLikeService.likeCourse(userId, courseId);
        return ApiResponse.success(SuccessStatus.COURSE_LIKE_SUCCESS);
    }

    @DeleteMapping("/{courseId}/like")
    @Override
    public ResponseEntity<ApiResponse<Void>> unlikeCourse(
            @AuthenticationPrincipal Long userId,
            @PathVariable Long courseId
    ) {
        courseLikeService.unlikeCourse(userId, courseId);
        return ApiResponse.success(SuccessStatus.COURSE_UNLIKE_SUCCESS);
    }
}
